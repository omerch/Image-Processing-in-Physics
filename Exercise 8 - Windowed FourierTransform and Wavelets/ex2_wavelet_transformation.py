...
thresholds the wavelet coefficients, performs the inverse wavelet
transform, in order to compress the image. The script plots the original
image, the wavelet tiling, the thresholded tiling, and the compressed image.

See IPPtools for details on the functions that have been added for this exercise:
dwt_multiscale
idwt_multiscale
tile_dwt
rescale
...
by the ???s) and hence generate a successfully compressed image.
...
# Choose your favourite wavelet type. See the pywt wavelet object homepage for a list
...
coeffs, (A, H, V, D) = IPPT.dwt_multiscale(im, nLevel=nLevel, mode=mode, wavelet=wavelet)
...
newcoeffs = [[iCoeffs*(iCoeffs**2 >= lowest_power) for iCoeffs in iLevels] for iLevels in coeffs]
...
print 'compression by %3.1f%% leads to %3.1f%% relative error' % (100-100*compression_level, 100*(1-power1/power0))    
...
plt.gray()
plt.subplot(2,2,3)
plt.imshow(IPPT.tile_dwt(coeffs, im.shape)**(1 / 4.))
plt.title('Wavelet decomposition (gamma 0.25)')
plt.gray()
plt.subplot(2,2,4)
plt.imshow(IPPT.tile_dwt(newcoeffs, im.shape)**(1 / 4.))
